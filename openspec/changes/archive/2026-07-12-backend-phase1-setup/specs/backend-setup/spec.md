...
### Requirement: Pool de conexão MySQL
O sistema SHALL manter um pool de conexões MySQL (`backend/config/database.js`, via `mysql2/promise`) com no máximo 10 conexões simultâneas (`connectionLimit: 10`), reutilizado por todos os models.
